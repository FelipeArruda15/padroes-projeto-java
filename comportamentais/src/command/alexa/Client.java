...
import command.alexa.alexa.Alexa;
import command.alexa.commands.TurnLightOffCommand;
import command.alexa.commands.TurnLightOnCommand;
...
public class Client {
    ...
    public static void configureAlexa(Alexa alexa) {
        PhillipsLight livingRoom = new PhillipsLight();
        XiaomiLight kitchen = new XiaomiLight();

        alexa.addIntegration("Turn on the Living room light", new TurnLightOnCommand(livingRoom), "on", "Living room",
                "light");
        alexa.addIntegration("Turn off the Living room light", new TurnLightOffCommand(livingRoom), "off",
                "Living room", "light");
